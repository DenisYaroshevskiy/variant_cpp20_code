...
class variant {
  ...
  friend constexpr auto&& detail::get_data(auto&& self);
...
constexpr auto visit(Vis&& vis, Vs&& ... vs)
  noexcept ( noexcept(detail::visit(TOOLS_FWD(vis), detail::get_data(TOOLS_FWD(vs)) ...)) ) ->
  decltype ( detail::visit(TOOLS_FWD(vis), detail::get_data(TOOLS_FWD(vs)) ...) )
{
  return detail::visit(TOOLS_FWD(vis), detail::get_data(TOOLS_FWD(vs)) ...);
}
